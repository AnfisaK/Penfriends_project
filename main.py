...
from forms import RegisterForm, LoginForm, ProfileForm, DeleteForm, NotesForm, ParametersForm, AddFriendsForm, \
    RequestForm
...
def register():  # регистрация
    ...
    if form.validate_on_submit():
        ...
        if session.query(users.User).filter(users.User.email == form.email.data).first():
            return render_template('reg.html', form=form, title="Registration", message="Email is already registered.")
        if int(form.age.data) <= 0:  # проверяем корректность возраста
            return render_template('reg.html', form=form, title="Registration", message="Age is incorrect.")
        if len(form.password.data) < 4:  # проверяем корректность пароля
            return render_template('reg.html', form=form, title="Registration", message="Password is unreliable.")
        if form.password.data != form.repeat_passw.data:  # сравниваем пароли
            return render_template('reg.html', form=form, message="Passwords are different", title="Registration")
        else:
            session.add(user)
            session.commit()
            return render_template('reg.html', form=form, message="Registration was successful", title="Registration")
    else:
...
def parameters_func():  # ставим параметры поиска
    ...
    if form.validate_on_submit():
        if ((form.age_from.data and int(form.age_from.data) <= 0)
            ...
            return render_template('parameters.html', title="Search parameters",
                                   form=form, message='Age is incorrect.')  # проверяем возраст на корректность
